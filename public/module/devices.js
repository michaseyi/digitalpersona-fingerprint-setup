;(function (global, factory) {
	typeof exports === "object" && typeof module !== "undefined"
		? factory(exports, require("@digitalpersona/core"), require("WebSdk"))
		: typeof define === "function" && define.amd
		? define(["exports", "@digitalpersona/core", "WebSdk"], factory)
		: ((global = global || self),
		  factory(
				((global.dp = global.dp || {}), (global.dp.devices = global.dp.devices || {})),
				global.dp.core
		  ))
	...
	class MultiCastEventSource {
		...
		_off(event, handler) {
			if (event) {
				...
				if (hh) {
					if (handler) this.handlers[event] = hh.filter((h) => h !== handler)
		...
		emit(event) {
			...
			if (unicast) this.invoke(unicast, event)
			const multicast = this.handlers[eventName]
			if (multicast) multicast.forEach((h) => this.invoke(h, event))
		}
	...
	class Channel {
		...
		send(request, timeout) {
			...
			if (this.webChannel.isConnected()) this.processRequestQueue()
			else this.webChannel.connect()
			return deferred
		}
		onConnectionSucceed() {
		...
		onDataReceivedTxt(data) {
			...
			if (message.Type === MessageType.Response) {
				...
				if (request !== null) {
					...
					if (hr > 0x7fffffff) request.reject(new Error(`0x${hr.toString(16)}`))
					else request.resolve(response)
				} else console.log(`Orphaned response: ${message.Type}`)
	...
	class CardsReader extends MultiCastEventSource {
		...
		enumerateReaders() {
			return this.channel
				.send(new Request(new Command(Method.EnumerateReaders)))
				.then((response) => {
		 ...
		 */
		enumerateCards() {
			return this.channel.send(new Request(new Command(Method.EnumerateCards))).then((response) => {
				const list = JSON.parse(core.Utf8.fromBase64Url(response.Data || "{}"))
		...
		getCardInfo(reader) {
			return this.channel
				.send(
					new Request(new Command(Method.GetCardInfo, core.Base64Url.fromJSON({ Reader: reader })))
				)
				.then((response) => {
		...
		getCardUid(reader) {
			return this.channel
				.send(
					new Request(new Command(Method.GetCardUID, core.Base64Url.fromJSON({ Reader: reader })))
				)
				.then((response) => {
				...
				.send(
					new Request(
						new Command(
							...
							core.Base64Url.fromJSON({ Reader: reader, PIN: pin || "" })
						)
					)
				)
				.then((response) => {
		...
		subscribe(reader) {
			return this.channel
				.send(
					new Request(
						new Command(Method.Subscribe, reader ? core.Base64Url.fromJSON({ Reader: reader }) : "")
					)
				)
				.then()
		}
			...
			return this.channel
				.send(
					new Request(
						new Command(
							Method.Unsubscribe,
							reader ? core.Base64Url.fromJSON({ Reader: reader }) : ""
						)
					)
				)
				.then()
	...
	class FingerprintReader extends MultiCastEventSource {
		...
		enumerateDevices() {
			return this.channel
				.send(new Request(new Command(Method$1.EnumerateDevices)))
				.then((response) => {
					if (!response) return []
		...
		getDeviceInfo(deviceUid) {
			return this.channel
				.send(
					new Request(
						new Command(Method$1.GetDeviceInfo, core.Base64Url.fromJSON({ DeviceID: deviceUid }))
					)
				)
				.then((response) => {
		...
		startAcquisition(sampleFormat, deviceUid) {
			return this.channel
				.send(
					new Request(
						new Command(
							Method$1.StartAcquisition,
							core.Base64Url.fromJSON({
		...
		processNotification(notification) {
			switch (notification.Event) {
				case NotificationType$1.Completed:
					...
					return this.emit(
						new SamplesAcquired(notification.Device, completed.SampleFormat, completed.Samples)
					)
	...
	class WindowsAuthClient extends MultiCastEventSource {
		...
		init() {
			return this.channel.send(new Request(new Command(Method$2.Init)), 3000).then((response) => {
		...
		continue(handle, data) {
			return this.channel
				.send(
					new Request(
						new Command(Method$2.Continue, JSON.stringify({ Handle: handle, Data: data }))
					)
				)
				.then((response) => {
		...
		term(handle) {
			return this.channel
				.send(new Request(new Command(Method$2.Term, JSON.stringify({ Handle: handle }))))
				.then()
		}
...
//# sourceMappingURL=index.umd.js.maps
